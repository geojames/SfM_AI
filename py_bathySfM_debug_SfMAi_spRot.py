...
def footprints(cam, sensor, base_elev):
    ...
    *** Photos that have picth angles that cause the horizon to be visable will
    ...
    footprints = pd.DataFrame(np.zeros([cam.shape[0],7]), 
                              columns=['fov','cam_pt','frame_gnd','frame_vectx',
                                       'frame_vecty','frame_vectz','frame_mag'])
    footprints.cam_pt = footprints.frame_gnd.astype('object')
    footprints.frame_gnd = footprints.frame_gnd.astype('object')
...
#        itp = np.zeros((cam.shape[0],4,2))
    ...
    for idx, row in cam.iterrows():
        ...
        # append inter_points as a matplotlib path object and cam_pt to footprints 
        footprints.fov[idx] = mplPath.Path(inter_points)
        footprints.cam_pt[idx] = cam_pt
        
        # calc the frame center ray with the gound plane
        frame_cent = spg.Point3D(s_pts[4,0],s_pts[4,1],s_pts[4,2])
        cent_ray = spg.Ray3D(frame_cent,cam_sp)
        frame_gnd = plane.intersection(cent_ray)
        footprints.frame_gnd[idx] = np.array([frame_gnd[0][0].evalf(),
                                              frame_gnd[0][1].evalf(),
                                              frame_gnd[0][2].evalf()])
    
        cam_vect = np.array([float(cam_pt[0]-frame_gnd[0][0].evalf()),
                             float(cam_pt[1]-frame_gnd[0][1].evalf()),
                             float(cam_pt[2]-frame_gnd[0][2].evalf())])
        
        footprints.frame_vectx[idx] = cam_vect[0]
        footprints.frame_vecty[idx] = cam_vect[1]
        footprints.frame_vectz[idx] = cam_vect[2]
        
        footprints.frame_mag[idx] = np.sqrt(cam_vect[0]**2 + 
                                            cam_vect[1]**2 + 
                                            cam_vect[2]**2)
...
def visibility(cam, sensor, footprints, targets):
    ...
    # cam_pts arrays, tar. point arrays, c_pt - pt_p = vector
    cam_px = np.atleast_2d(cam.x.values)
    cam_py = np.atleast_2d(cam.y.values)
    cam_pz = np.atleast_2d(cam.z.values)
    pt_px = np.atleast_2d(targets.x.values).T
    pt_py = np.atleast_2d(targets.y.values).T
    pt_pz = np.atleast_2d(targets.z.values).T
    
    pt_vx = cam_px - pt_px
    pt_vy = cam_py - pt_py
    pt_vz = cam_pz - pt_pz
    pt_mag = np.sqrt(pt_vx**2 + pt_vy**2 + pt_vz**2)
    
    frame_vx = np.repeat(np.atleast_2d(footprints.frame_vectx),targets.shape[0],axis=0)
    frame_vy = np.repeat(np.atleast_2d(footprints.frame_vecty),targets.shape[0],axis=0)
    frame_vz = np.repeat(np.atleast_2d(footprints.frame_vectz),targets.shape[0],axis=0)
    frame_mag = np.repeat(np.atleast_2d(footprints.frame_mag),targets.shape[0],axis=0)
    
    dot = (frame_vx * pt_vx) + (frame_vy * pt_vy) + (frame_vz * pt_vz)
    comb_mag = frame_mag * pt_mag
    
    f_ang = np.arccos(dot/comb_mag)
    f_ang = f_ang * vis
    f_ang[f_ang == 0] = np.nan
    ...
    rad_dist = f_pix[0] * np.tan(f_ang)
    rad_dist = rad_dist * vis
    rad_dist[rad_dist == 0] = np.nan
    
    max_rad_dist = np.sqrt(sensor.pix_x**2 + sensor.pix_y**2)
    per_rad_dist = rad_dist / max_rad_dist[0]
...
def main_prog():
    ...
    # output filename
    outfile = 'D:/Dropbox/Python/SfMAI/data/Mochlos_Test4_out.csv'
